...
app.post("/compare", async (req, res) => {
	...
	try {
		...
		const message = `Nelson's average GPA for 335 is: ${avg_gpa.toFixed(2)}. Your GPA is ${Math.abs(offset).toFixed(2)} ${more_less} than Nelson's average.<br>${words}`;
		res.json({ message });
